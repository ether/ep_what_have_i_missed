# Agent Guide — ep_what_have_i_missed
...
`ep_what_have_i_missed` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_what_have_i_missed`:
...
* Server: `clientVars`, `eejsBlock_mySettings`, `eejsBlock_body`, `eejsBlock_customStyles`
