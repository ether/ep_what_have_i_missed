# Contributing to ep_what_have_i_missed

Thanks for helping improve `ep_what_have_i_missed` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_what_have_i_missed` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_what_have_i_missed from your local clone
pnpm run plugins i --path ../ep_what_have_i_missed

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_what_have_i_missed/` are picked up after a server restart.
...
* Etherpad version, ep_what_have_i_missed version, browser + OS
* Steps to reproduce
